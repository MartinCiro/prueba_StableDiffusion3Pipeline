...
class FullBodyGenerator:
    ...
    """Generador con modelo MEDIUM (SD 2.1) - Mejor calidad"""
    ...
    def __init__(self):
        if not hasattr(self, 'initialized'):
            print("🤖 Iniciando Generador MEDIUM de Cuerpo Completo...")
            
            # Configurar para mínimo consumo
            environ['TOKENIZERS_PARALLELISM'] = 'false'
            environ['PYTORCH_CUDA_ALLOC_CONF'] = 'max_split_size_mb:32'
            
            # Forzar CPU si no hay suficiente memoria GPU
            if cuda.is_available():
                gpu_memory = cuda.get_device_properties(0).total_memory / (1024**3)
                self.device = "cuda" if gpu_memory >= 6 else "cpu"  # SD 2.1 necesita más memoria
            else:
                self.device = "cpu"
            ...
            # Directorios
            self.output_dir = Path(getenv('OUTPUT_DIR', './fullbody_medium_outputs'))
            self.output_dir.mkdir(exist_ok=True)
            
            # MODELO MEDIUM (SD 2.1) - ¡CAMBIO AQUÍ!
            ...
            # PROMPTS OPTIMIZADOS PARA SD 2.1
            self.prompts = {
                "fullbody_portrait": {
                    "prompt": "professional full body portrait of a person standing, complete figure visible from head to toes, natural pose, clean background, photorealistic, detailed clothing, sharp focus, 8k",
                    "negative": "cropped, cut off, partial body, only face, close-up, blurry, deformed, missing limbs, bad anatomy, extra fingers, mutated hands",
                    "steps": 25,  # Un poco más para mejor calidad
                    "width": 384,
                    "height": 512
                },
                "fullbody_realistic": {
                    "prompt": "hyperrealistic full body photograph of person, cinematic lighting, detailed skin texture, natural shadows, professional photography, entire body in frame",
                    "negative": "cartoon, anime, 3d render, painting, drawing, blurry, pixelated, low quality, deformed proportions",
                    "steps": 30,
                    "width": 384,
                    "height": 512
                },
                "fullbody_simple": {
                    "prompt": "full body of person standing, plain background, simple pose, clearly visible from head to toes",
                    "negative": "cropped, partial, blurry, complex background, multiple people",
                    "steps": 20,
                    "width": 384,
                    "height": 512
                }
            ...
            print(f"✅ Generador MEDIUM inicializado (modelo: {self.model_id})")
    
    @lru_cache(maxsize=1)
    def _load_model(self):
        """Carga el modelo MEDIUM"""
        print(f"🔧 Cargando modelo MEDIUM ({self.model_id})...")
        
        try:
            # SD 2.1 funciona mejor con float32 incluso en CPU
            dtype = float32
            ...
            pipe = StableDiffusionPipeline.from_pretrained(
                ...
                use_safetensors=True  # SD 2.1 usa safetensors
            )
            
            # Optimizaciones para SD 2.1
            pipe.to(self.device)
            pipe.enable_attention_slicing(1)
            
            # SD 2.1 funciona mejor con DPMSolver++ con Karras sigmas
            pipe.scheduler = DPMSolverMultistepScheduler.from_config(
                pipe.scheduler.config,
                algorithm_type="dpmsolver++",
                use_karras_sigmas=True  # Mejor para SD 2.1
            )
            
            print(f"✅ Modelo MEDIUM cargado: {self.model_id}")
            return pipe
            
        except Exception as e:
            print(f"❌ Error cargando modelo MEDIUM: {e}")
            
            # Fallback a SD 1.5 si falla
            print("🔄 Intentando con SD 1.5 como fallback...")
            self.model_id = "runwayml/stable-diffusion-v1-5"
            return self._load_fallback()
    
    def _load_fallback(self):
        """Fallback a SD 1.5 si SD 2.1 falla"""
        try:
            pipe = StableDiffusionPipeline.from_pretrained(
            ...
            pipe.to(self.device)
            pipe.enable_attention_slicing(1)
            print("✅ Modelo fallback (SD 1.5) cargado")
            return pipe
        except Exception as e:
